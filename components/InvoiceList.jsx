...
export default function InvoiceList({
  ...
  const displayInvoices = useMemo(() => {
    return mergedInvoices.map((inv) =>
      editedInvoices[inv.id] ? { ...inv, ...editedInvoices[inv.id] } : inv
    );
  }, [mergedInvoices, editedInvoices]);
  ...
  return (
    <section aria-labelledby="invoice-list-heading" className="space-y-4">
      ...
      {invoices === null && displayInvoices.length === 0 ? (
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      ...
                      aria-label={
                        copy.invoices.editRowAriaLabel?.replace("{id}", invoice.id) ||
                        `Edit invoice ${invoice.id}`
                      }
                      className="rounded-xl border border-slate-700 bg-slate-800/80 px-3 py-1.5 text-xs font-semibold text-slate-300 transition-all hover:border-slate-600 hover:bg-slate-800 hover:text-slate-100 focus-ring"
                    >
                      {copy.invoices.editRowAction || "Edit"}
                    </button>
                  </div>
                </div>
