...
export function useWatchlist() {
  ...
  const toggleInvoice = useCallback((watchlistId, invoiceId) => {
    setWatchlists((prev) => {
      let current = Array.isArray(prev) ? prev : [];
      if (current.length === 0) {
        // Auto-create default watchlist and add invoice
        return [{ id: Date.now().toString(), name: "Default Watchlist", invoiceIds: [invoiceId] }];
      }
  ...
  // Clean up stale IDs
  const pruneStaleInvoices = useCallback((validInvoiceIds) => {
    setWatchlists((prev) => {
      if (!Array.isArray(prev)) return [];
      let changed = false;
