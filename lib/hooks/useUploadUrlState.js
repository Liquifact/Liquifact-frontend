...
/**
 * @typedef {object} UploadFilters
 * @property {string} [searchTerm]
 * @property {{key: string, direction: 'asc' | 'desc'}} sort
 */

/**
 * Custom hook to synchronize upload filter and sort state with URL query parameters.
 * It reads the initial state from the URL, and updates the URL as the state changes.
 *
 * @returns {[UploadFilters, (newFilters: Partial<UploadFilters>) => void]}
 */
export function useUploadUrlState() {
