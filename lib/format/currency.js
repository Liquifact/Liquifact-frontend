...
function resolveSupportedLocale(locale) {
  if (typeof locale !== "string" || locale.trim() === "") {
    return FORMAT_CONFIG.defaultLocale;
  }

  const requestedLocale = locale.trim();

  try {
    return Intl.NumberFormat.supportedLocalesOf(requestedLocale).length > 0
      ? requestedLocale
      : FORMAT_CONFIG.defaultLocale;
  } catch {
    return FORMAT_CONFIG.defaultLocale;
  }
}
...
function createFormatter(locale, options) {
  ...
  try {
    return new Intl.NumberFormat(supportedLocale, options);
  } catch {
